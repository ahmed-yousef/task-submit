...
work_path = os.path.dirname(os.path.realpath(__file__))#work directory path
#credential_dir = os.path.join(work_path, '.configs')#credentials or configs it doesnot matter
        ...
        if (not creds_found == '0') or (not creds_found == False):
                ...
                if not check.status_code==200:
                        ...
                        payload={'client_id':client_saved['web']['client_id'],\
                          ...
                          'refresh_token':get_prop(client,'refresh_token','admin'),\
                        ...
                        if req.status_code == 200:
                                ...
                                update_admin(client, 'admin', 'token', mid['access_token'])
...
client=create_client('assignment-submit')#data store client 
...
def get_prop(client,prop,user):#i use this for admin log in and retrive tokens
...
def load_client_secret():#discard this function
...
try:
    ...
    print('can not make the temp folder')
...
@app.route('/abstract.jpg')
def background():
    return send_from_directory(os.path.join(app.root_path, 'static'),'abstract.jpg', mimetype='image/jpeg')
...
def callback():
    ...
    #client=create_client('assignment-submit')
    update_admin(client, 'admin', 'token',cred['token'])
    update_admin(client, 'admin', 'refresh_token',cred['refresh_token'])
...
##                f.close
                update_admin(client, 'admin', 'drive_folder_id',drive_folder_id)
                print('success')
...
def clear_credentials():
    if 'username' in session:
      #client=create_client('assignment-submit')
      if get_prop(client,'token','admin'):
        update_admin(client, 'admin', 'token', '0')
        update_admin(client, 'admin', 'refresh_token', '0')
      return redirect(url_for('logged'))
    return 'You are not logged in <br><a href = "/admin"></b>' + \
      'click here to log in</b></a>'
        ...
        if not False in files_test:
           ...
           for file in files:#actual save of files
            ...
            if file and allowed_file(file.filename):#we should notify user of not saving same name files
                ...
                if not os.path.exists(filedir) and not check_file_name(filename_cloud):
